...
import { Building2, CalendarDays, CreditCard, LogOut, Users } from "lucide-react";
import { cn } from "@/shared/lib/utils";
import { Button } from "@/components/ui/button";
import { useAuth } from "@/features/auth/hooks/use-auth";

const navItems = [
  { label: "Members", to: "/org/members", icon: Users },
  { label: "Classes", to: "/org/classes", icon: CalendarDays },
  { label: "Billing", to: "/org/billing", icon: CreditCard },
] as const;
...
export function OrgNav({ organizationName }: OrgNavProps) {
  ...
  return (
    <aside className="w-60 shrink-0 border-r border-border/60 bg-card/40 flex flex-col h-full overflow-hidden">
      ...
      <div className="px-5 pt-6 pb-2 flex items-center gap-2">
        ...
        <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-primary truncate">
          {organizationName}
        </span>
      </div>
