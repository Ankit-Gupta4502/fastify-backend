...
check_env_file() {
    if [ ! -f "$ENV_FILE" ]; then
        echo -e "${RED}Error: root .env file not found at $ENV_FILE!${NC}"
        echo -e "${YELLOW}Please create a monorepo root .env file with the following required variables:${NC}"
        echo "  NODE_ENV=production"
        echo "  DATABASE_URL=postgresql://user:password@postgres:5432/yoga-app"
        echo "  BETTER_AUTH_SECRET=your-super-secret-key-minimum-32-characters"
        echo "  PORT=8080"
        echo "  POSTGRES_USER=postgres"
        echo "  POSTGRES_PASSWORD=your-secure-password"
        echo "  POSTGRES_DB=yoga-app"
        exit 1
    fi
    ...
    # Warn about insecure BETTER_AUTH_SECRET
    if [ "$BETTER_AUTH_SECRET" = "dev-secret" ] || [ ${#BETTER_AUTH_SECRET} -lt 32 ]; then
        echo -e "${YELLOW}Warning: BETTER_AUTH_SECRET appears to be insecure or too short (minimum 32 characters recommended)${NC}"
    fi
}
...
migrate() {
    ...
    MIGRATE_IMAGE_NAME="${MIGRATE_IMAGE_NAME:-social-sync-backend-migrate}"
    (cd "$BACKEND_DIR" && docker build --target build -t "$MIGRATE_IMAGE_NAME" -f Dockerfile .)
...
show_help() {
    ...
    echo "  ./scripts/docker-prod.sh restore backups/yoga-app_backup_20240101_120000.sql.gz"
}
