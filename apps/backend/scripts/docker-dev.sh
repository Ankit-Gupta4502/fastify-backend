...
check_env_file() {
    if [ ! -f "$ENV_FILE" ]; then
        echo -e "${YELLOW}Warning: root .env file not found at $ENV_FILE.${NC}"
        echo -e "${YELLOW}Create the monorepo root .env before starting Docker services.${NC}"
        echo "  NODE_ENV=development"
        echo "  DATABASE_URL=postgresql://postgres:postgres@postgres:5432/yoga-app-dev"
