...
**All scripts automatically load environment variables from the monorepo root `.env` file** if it exists. The scripts:
...
```bash
# 1. Ensure the monorepo root `.env` file exists
cat > ../../.env <<'EOF'
...
```bash
# 1. Ensure the root `.env` file has production values
# Verify BETTER_AUTH_SECRET is secure (32+ characters)
